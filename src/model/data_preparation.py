...
def split_data(data: pd.DataFrame, test_size: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    1. Split the data into a full training set (full_training_dataset) and test set (dataframe_test) using test_size.
    2. Split full_training_dataset into training (dataframe_train) and validation (dataframe_validation_features) sets.
    ...
    full_training_dataset, dataframe_test = train_test_split(data, test_size=test_size, random_state=1)
    dataframe_train, dataframe_validation_features = train_test_split(full_training_dataset, test_size=0.25, random_state=1)
    ...
    return dataframe_train, dataframe_validation_features, dataframe_test, full_training_dataset
